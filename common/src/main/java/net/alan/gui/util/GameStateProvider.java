...
public class GameStateProvider {
    ...
    public static void injectScreenVariables(Map<String, Integer> vars, Minecraft mc) {
        vars.put("game.in_level", resolve("game.in_level"));
        vars.put("game.is_singleplayer", resolve("game.is_singleplayer"));
    }
    ...
    public static int resolve(String key) {
        ...
        return switch (key) {
            ...
            case "game.has_cheats" -> {
                ...
                yield mc.player.isCreative() ? 1 : 0;
            }
            case "game.is_host" -> {
                if (mc.hasSingleplayerServer()) {
                    IntegratedServer server = mc.getSingleplayerServer();
            ...
            case "game.is_op" -> mc.player != null && mc.player.isCreative() ? 1 : 0;

            case "game.is_connected" -> mc.getConnection() != null ? 1 : 0;
            case "game.is_realm" -> 0;
